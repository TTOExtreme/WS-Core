...
ClientEvents.emit("LoadExternal", [
    ...
    "./module/WSOP/js/vendas/addproduto.js",
...
window.UserList = class UserList {
    ...
    actionIcon = function (cell, formatterParams, onRendered) { //plain text value
        ...
        if (Myself.checkPermission("WSOP/os/edt") && new window.Modules.WSOP.StatusID().enableEdit(cell.getRow().getData().status, "vendas")) {
            let bot = document.createElement("i");
        ...
        if (Myself.checkPermission("WSOP/os/edt") && (new window.Modules.WSOP.StatusID().enableEdit(cell.getRow().getData().status, "vendas") || Myself.checkPermission("WSOP/menu/vendas/all"))) {
            let bot = document.createElement("i");
