...
function getBaseIp(opts) {
    opts.ip = ipList.normalizeIP3(opts.ip);
    if (!('ip' in opts)) {
...
module.exports = {
    NetScan: function scan(opts, callback, done) {
        var finalOpts = getBaseIp(opts);
        Object.assign(options, finalOpts);
        // console.log(options);

        ipList.GetList(options.ip, options.netmask, (ips) => {
            var promise = ips.map((ip) => {
                return getPromiseInfo(ipList.normalizeIP4(ip), callback);
            })
...
function getPromiseInfo(ip, callback) {
