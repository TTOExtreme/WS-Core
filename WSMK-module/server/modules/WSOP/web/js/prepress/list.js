...
window.UserList = class UserList {
    ...
    actionIcon = function (cell, formatterParams, onRendered) { //plain text value
        ...
        if (Myself.checkPermission("WSOP/os/edt") && new window.Modules.WSOP.StatusID().enableEdit(cell.getRow().getData().status, "prepress")) {
            let bot = document.createElement("i");
    ...
    newCollums = [{
        ...
        columns: [
            ...
            {
                title: 'Valor', field: 'status', headerFilter: "select", headerFilterParams: this._getStatusFilterParams(),
                formatter: function (cell) {
                    let datacell = cell.getRow().getData();
                    let total = 0.00;//valor total
                    if (datacell.produtos != undefined) {
                        datacell.produtos.forEach((produto) => {
                            total += (produto.qnt * parseFloat(produto.price.replace(",", ".").replace(" ", "")));
                        });
                    }

                    return parseFloat(total).toFixed(2) + " R$";
                }
            },
    ...
    constructor() {
        ...
        this.main_table.setFilter([{ field: "status", in: new window.Modules.WSOP.StatusID().getStatusSector("prepress") }]);
