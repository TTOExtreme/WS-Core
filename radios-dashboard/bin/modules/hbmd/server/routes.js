...
function RoutesInit(RouteAdd) {
    ...
    RouteAdd("hbmd/top/menu/cpu", "hbmd/top/menu/cpu", (UUID, user_id, data, returnData) => {
        require(path.join(__dirname + '/rotine/sql/select/menu/cpu-top.js'))(UUID, (ret) => {
            if (ret.status == "ERROR") { returnData({ route: "system/error", data: ret }); return; }
            returnData({ route: "hbmd/list/cpu/menu", data: ret });
        });
    });
    RouteAdd("hbmd/top/menu/mem", "hbmd/top/menu/mem", (UUID, user_id, data, returnData) => {
        require(path.join(__dirname + '/rotine/sql/select/menu/mem-top.js'))(UUID, (ret) => {
            if (ret.status == "ERROR") { returnData({ route: "system/error", data: ret }); return; }
            returnData({ route: "hbmd/list/mem/menu", data: ret });
        });
    });
    ...
    RouteAdd("hbmd/top/menu/network", "hbmd/top/menu/network", (UUID, user_id, data, returnData) => {
        require(path.join(__dirname + '/rotine/sql/select/menu/network-top.js'))(UUID, (ret) => {
            if (ret.status == "ERROR") { returnData({ route: "system/error", data: ret }); return; }
            returnData({ route: "hbmd/list/network/menu", data: ret });
        });
    });


    //load subnets list
    RouteAdd("hbmd/get/subnets", "hbmd/get/subnets", (UUID, user_id, data, returnData) => {
        require(path.join(__dirname + '/rotine/sql/select/Subnet.js'))((ret) => {
            if (ret.status == "ERROR") { returnData({ route: "system/error", data: ret }); return; }
            returnData({ route: "hbmd/list/subnets", data: ret });
        });
    });
