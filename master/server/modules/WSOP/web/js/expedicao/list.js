...
window.UpdateMainTable = setInterval(() => {
    ClientEvents.emit("SendSocket", "wsop/os/lst", { status: new window.Modules.WSOP.StatusID().getStatusSector("expedicao") });
}, 10 * 1000);
...
window.UserList = class UserList {
    ...
    constructor() {
        ...
        this.main_table.setFilter([{ field: "status2", type: "in", value: new window.Modules.WSOP.StatusID().getStatusSector("expedicao") }]);
        this._init();
        ClientEvents.emit("SendSocket", "wsop/os/lst", { status: new window.Modules.WSOP.StatusID().getStatusSector("expedicao") });
    }
    ...
    _init() {
        ...
        ClientEvents.on("system/edited/os", () => { ClientEvents.emit("system_mess", { status: "OK", mess: "OS Editada com Exito", time: 1000 }); ClientEvents.emit("SendSocket", "wsop/os/lst", { status: new window.Modules.WSOP.StatusID().getStatusSector("expedicao") }); });
    }
