...
const exe = (subnet, callback) => {
    var ip = ipList.normalizeip(subnet.data.ip);
    var netmask = ipList.normalizeNetmask(subnet.data.netmask);

    var sql = "DELETE FROM " + dbstruct.database + "._Subnets WHERE `ip`='" + subnet.data.ip + "';";
