...
ClientEvents.on("grp/edt", (data) => {
    ...
    div.innerHTML = "" +
        ...
        "<tr><td class='grp_edt_label'>Usuário:</td><td><input type='text' disabled value='" + data.username + "'></td></tr>" +
        ...
        "<tr><td class='grp_edt_label'>Conectado:</td><td><input type='text' disabled value='" + ((data.connected == 1) ? "Sim" : "Não") + "'></td></tr>" +
        "<tr><td class='grp_edt_label'>Ultimo Login:</td><td><input type='text' disabled value='" + formatTime(data.lastConnection) + "'></td></tr>" +
        "<tr><td class='grp_edt_label'>Ultima Tentativa:</td><td><input type='text' disabled value='" + formatTime(data.lastTry) + "'></td></tr>" +
        "<tr><td class='grp_edt_label'>Ultimo IP:</td><td><input type='text' disabled value='" + ((data.lastIp != null) ? data.lastIp : "-") + "'></td></tr>" +
...
ClientEvents.on("grp/edt/save", () => {
    ClientEvents.emit("SendSocket", "adm/grp/edt/save", {
        id_user: document.getElementById("grpedtid").value,
