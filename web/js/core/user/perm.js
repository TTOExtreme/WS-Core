...
ClientEvents.on("usr/perm", (data) => {
    ...
    div.innerHTML = "" +
        ...
        "<tr><td colspan=2><div id='perm_table' class='tabulator' style='max-width: 80vw;height: 450px;'></div></td></tr>" +
        "</table>";
    ...
    table_usr_perm = new Tabulator("#perm_table", {
        ...
        columns: [
            {
                title: 'Status', field: 'active', formatter: "tickCross", headerFilter: "select", headerFilterParams:
                ...
                cellClick: function (e, cell) {
                    ...
                    if (confirm("Voce esta prestes a " + ((row['active'] == 1) ? "Desvincular" : "Vincular") + " a Permissão ao Usuário: " + data.username + "\nVoce tem certeza disso?")) {
                        ...
                        ClientEvents.emit("SendSocket", "adm/usr/perm/set", { id_user: data.id, code: row.code, active: ((row.active == 1) ? 1 : 0) });
                    }
            ...
            { title: 'Adicionado Em', field: 'createdIn', formatter: ((data) => formatTime(data.getRow().getData().createdIn)), headerFilter: "input" },
            { title: 'Adicionado Por', field: 'createdBy', headerFilter: "input" },
