    ...
    add(name, description, createdBy, route, subdomain, folder, log, active, deleted) {
        return new Promise((res, rej) => {
            var sql = "INSERT INTO " + this._cfg.dbstruct.database +
                "._WPMA_Sites (name,description,createdBy,route,subdomain,folder,log,active,deleted) VALUES " +
                "('" + name + "','" + description + "'," + createdBy + ",'" + route + "','" + subdomain + "','" + folder + "'," + log + "," + active + "," + deleted + ");";
            this._db.query(sql)
                .then(() => {
                    res();
                })
